...
public class AuthFilter {
    ...
    public Mono<MemberInfoDto> authenticate(ServerRequest request){
        ...
        return webClient.get().uri("/auth/check").retrieve()
    ...
    private String getBearerToken(ServerRequest request){
        List<String> list = request.headers().header("Authorization");
        if(list.isEmpty()){
            log.error("Jwt Token is not exist !");
            throw new RuntimeException("Jwt Token is not exist !");
        }
